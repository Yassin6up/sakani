...
import Animated, { ColorSpace, FadeInRight, FadeOutLeft } from 'react-native-reanimated';
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {  
        const response = await axios.get(`https://backend.sakanijo.com/api/user/${result}/likes`);
      ...
      <Link key={item.id} href={`/listing/${item.id}`} asChild>
        <TouchableOpacity>
          <Animated.View style={styles.listing} entering={FadeInRight} exiting={FadeOutLeft}>
            <Animated.Image source={{ uri: `https://backend.sakanijo.com/api/images/${encodeURIComponent(item.folderName)}/${encodeURIComponent(photos[0])}` }} style={styles.image} />
            <TouchableOpacity style={{ position: 'absolute', right: 30, top: 30 }}>
              <Ionicons name="heart" size={24} color={Colors.primary} />
            </TouchableOpacity>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ fontSize: 16, fontFamily: 'droidAr' }}>{item.title}</Text>
            ...
            <View style={{ flexDirection: 'row', gap: 4 }}>
            ...
            
              {
                ...
                <Text style={{ fontFamily: 'droidAr' }}>اليلة</Text>
                :
                null
              }
