  ...
  const handleCloseModal = () => setModalVisible(false);
    ...
    const handleSubmitCode = async (code) => {
      ...
      try {
        ...
        const response = await fetch('https://backend.sakanijo.com/verify-phone', {
          method: 'POST',
          headers: {
    ...
    async function register(phoneNumber, password , name) {
      try {
        ...
        console.log("pass sended :" , password)
        // Send the phone number and password to the backend
        const response = await axios.post('https://backend.sakanijo.com/register', {
          phone: formattedPhoneNumber,
          password: password ,
          name : name
        });
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        style={[defaultStyles.inputField, { marginBottom: 30  , fontFamily : "droidAr" }]}
      ...
      <Pressable onPress={()=>router.navigate("/ForgetPasscode")}>
        <Text style={{fontFamily : "droidAr" , color: "blue" , padding : 10 , fontSize : 14}}>هل نسيت كلمة المرور ؟</Text>
      </Pressable>
      <TouchableOpacity style={defaultStyles.btn} onPress={()=>{
        login(phone,password)
      }}>
        <Text style={{...defaultStyles.btnText , fontFamily : "droidAr"}}>تسجيل الدخول</Text>
      </TouchableOpacity>
        </>
...
<>
    ...
    <TextInput
      ...
      <TouchableOpacity style={defaultStyles.btn} onPress={()=>{
        ...
        <Text style={{...defaultStyles.btnText , fontFamily : "droidAr"}}>انشاء حساب</Text>
      </TouchableOpacity>
